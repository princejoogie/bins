...
if [ -f "$GAMEFILE" ]; then
  ...
  if [ $DISPLAY_WIDTH -le 1279 ]; then # increase the scale for smaller screens
    ...
    # sed -i 's/self.TILESCALE = .*/self.TILESCALE = 4.65/g' globals.lua
    sed -i 's/self.TILE_W = .*/self.TILE_W = 21/g' globals.lua
    sed -i 's/self.TILE_H = .*/self.TILE_H = 11.5/g' globals.lua
    sed -i 's/self.CARD_W = .*/self.CARD_W = 2.5*35\/41/g' globals.lua
    sed -i 's/self.CARD_H = .*/self.CARD_H = 2.5*47\/41/g' globals.lua
